<?php 
	...
	function get_ThursdayData() { 
        $file = fopen(__DIR__."/../data/EventsScheduleThursday.csv","r") or die("Unable to connect to open file");
        $time = [];
        $events = [];
        $location = [];
		while(!feof($file)) { 
	...
	function display_ThursdayTable($time, $events, $location) {
		echo "<table class='tg'>";
	...
	list($time, $events, $location) = get_ThursdayData(); //each element of array is assigned to variable of list
	display_ThursdayTable($time, $events, $location);
