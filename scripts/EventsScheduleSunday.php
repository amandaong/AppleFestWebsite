<?php 
	...
	function get_SundayData() { 
        $file = fopen(__DIR__."/../data/EventsScheduleSunday.csv","r") or die("Unable to connect to open file");
        $time = [];
        $events = [];
        $location = [];
		while(!feof($file)) { 
	...
	function display_SundayTable($time, $events, $location) {
		echo "<table class='tg'>";
	...
	list($time, $events, $location) = get_SundayData(); //each element of array is assigned to variable of list
	display_SundayTable($time, $events, $location);
