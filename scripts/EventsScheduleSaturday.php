<?php 
	...
	function get_SaturdayData() { 
        $file = fopen(__DIR__."/../data/EventsScheduleSaturday.csv","r") or die("Unable to connect to open file");
        $time = [];
        $events = [];
        $location = [];
		while(!feof($file)) { 
	...
	function display_SaturdayTable($time, $events, $location) {
		echo "<table class='tg'>";
	...
	list($time, $events, $location) = get_SaturdayData(); //each element of array is assigned to variable of list
	display_SaturdayTable($time, $events, $location);
